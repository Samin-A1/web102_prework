...
function addGamesToPage(GAMES_JSON) {
    ...
    GAMES_JSON.forEach((game) => {
        ...
        const template = `
            <p><img src = ${game.img} class = "game-img";"></img></p>
            <h3>${game.name}</h3>
...
const sumOfUnfundedGames = GAMES_JSON.reduce((acc, game) => {
    return (game.pledged < game.goal ? acc + 1 : acc);
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const explainUnfunded = `
A total of <span style="color:lime;">$${totalRaised.toLocaleString('en-US')}</span> has been raised for <span style="color:lime;">${totalGames}</span> games. 
Currently, <span style="color:red;">${sumOfUnfundedGames}</span> ${sumOfUnfundedGames === 1 ? "game remains" : "games remain"} unfunded. 
We need your help to fund these amazing games!
`;

// create a new DOM element containing the template string and append it to the description container
let unfundedMessage = document.createElement('div');
unfundedMessage.classList.add("description-container");
unfundedMessage.innerHTML = explainUnfunded;
